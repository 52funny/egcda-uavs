...
impl codec::Encoder<UavGsCommunicateRequest> for UavGsCommunicateCodec {
    type Error = std::io::Error;
    fn encode(
        &mut self,
        item: UavGsCommunicateRequest,
...
impl codec::Decoder for UavGsCommunicateCodec {
    type Item = UavGsCommunicateResponse;
    type Error = std::io::Error;
    fn decode(&mut self, src: &mut bytes::BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        let item = UavGsCommunicateResponse::decode(&data[4..])?;
        Ok(Some(item))
    }
}
